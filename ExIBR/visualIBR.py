...
class IBR(object):
    ...
    def get_cost_updates(self, learning_rate):
        ...
        s_ij = T.mean(
            ((T.dot(self.xi, self.Y) - T.dot(self.xj, self.Y)) ** 2), 1
        )
        s_ik = T.mean(
            ((T.dot(self.xi, self.Y) - T.dot(self.xk, self.Y)) ** 2), 1
        )
#        theta_ij = T.log(T.nnet.sigmoid(-s_ij + self.c))
#        theta_ik = T.log(1 - T.nnet.sigmoid(-s_ik + self.c))
#        theta_ij = T.tanh(-s_ij + self.c) //good performance
...
def test_dA(epoch_time):
    ...
    train_set_xi, train_set_xj, train_set_xk = theano.shared(train_set[0]), theano.shared(train_set[1]), theano.shared(train_set[2])
    ...
    for batch_size in [100]:
        for learning_rate in [1]:
            for y in [500, 250, 300, 100, 600, 400]:
                ...
                for epoch in range(epoch_time):
                    ...
                    print 'learning_rate is %f, batch_size is %f, y is %f, %i epochs train ended, cost is %f, s_ij is %f, s_ik is %f' % (learning_rate, batch_size, y, epoch, cost, s_ij, s_ik)

                    fi.write('learning_rate is %f, batch_size is %f, y is %f, %i epochs train ended, cost is %f, s_ij is %f, s_ik is %f' % (learning_rate, batch_size, y, epoch, cost, s_ij, s_ik))
                    fi.flush()
                    ...
                    def valid_model2(size, k):
                        ...
                        for i in range(valid_data.shape[0]):
                            ...
                            vij = numpy.sum(
                                (numpy.dot(numpy.asarray(vi[xi]), y) - numpy.dot(numpy.asarray(vj[xj]), y)) ** 2
                            )
                            compare.append(vij)
                            for j in range(k-1):
                                xk =int(valid_data[i][j+2])
                                vik = numpy.sum(
                                    (numpy.dot(numpy.asarray(vi[xi]), y) - numpy.dot(numpy.asarray(vk[xk]), y)) ** 2
                                )
                                compare.append(vik)
                    ...
                    def test_model2(size, k):
                        ...
                        for i in range(test_data.shape[0]):
                            ...
                            tij = numpy.sum(
                                (numpy.dot(numpy.asarray(ti[xi]), y) - numpy.dot(numpy.asarray(tj[xj]), y)) ** 2
                            )
                            compare.append(tij)
                            for j in range(k - 1):
                                xk = int(test_data[i][j + 2])
                                tik = numpy.sum(
                                    (numpy.dot(numpy.asarray(ti[xi]), y) - numpy.dot(numpy.asarray(tk[xk]), y)) ** 2
                                )
                                compare.append(tik)
                    ...
                    print 'learning_rate is %f, batch_size is %f, y is %f, %i epoch, validation score is %f' % (iter[0], iter[1], iter[2], epoch, this_validation_score)
                    ...
                    if this_validation_score > best_validation_score:
                        ...
                        best_iter = iter
                        numpy.savetxt('ibr_y.csv', ibr.Y.get_value(), fmt="%f")
                        print 'test start'
