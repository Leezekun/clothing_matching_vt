...
def train_model(size, k):
    train_id = numpy.loadtxt('train_ijk_811_noshuffle_10000.txt', delimiter="\t")
    f1 = open('train_k_'+str(k)+'_10000.txt','w+')
    f2 = open('train_k_' + str(k) + '_id_10000.txt', 'w+')
    ...
    for i in range(top.__len__()):
        ...
        compare.append(positive[0])
        compare_id.append(train_id[positive[0]][0])
        compare_id.append(train_id[positive[0]][1])
        k_list = random.sample(range(size), 3 * k)
        for j in k_list:
            if (vk_list[j] not in positive_v):
                compare.append(j)
                compare_id.append(train_id[j][2])
            if (compare.__len__() == k+1):
                break
        train_data.append(compare)
        train_data_id.append(compare_id)
    ...
    numpy.savetxt('train_k_10_10000.csv', train_data, fmt="%d")
    numpy.savetxt('train_k_10_id_10000.csv', train_data_id, fmt="%d")
...
def test_model(size, k):
    test_id = numpy.loadtxt('test_ijk_811_noshuffle_10000.txt', delimiter="\t")
