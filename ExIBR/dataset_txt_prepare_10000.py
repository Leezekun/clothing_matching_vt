...
my_matrix_text_bottom = numpy.loadtxt(
    open("/storage/songxuemeng/lizekun/ExIBR/bottom_feature_new.txt", "r"), delimiter=",",
    skiprows=0)
print ('bottom shape')
...
my_matrix_text_top = numpy.loadtxt(
    open("/storage/songxuemeng/lizekun/ExIBR/top_feature_new.txt", "r"), delimiter=",",
    skiprows=0)
...
print   ('now():' + str(datetime.now()))
ijk_list = numpy.loadtxt(open("/storage/songxuemeng/lizekun/ExIBR/train_ijk_811_noshuffle_10000.txt", "r"),
                         delimiter="\t", skiprows=0)
...
write_file = open('/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_train8110_10000.pkl',
                  'wb')
...
print   ('now():' + str(datetime.now()))
ijk_list = numpy.loadtxt(open("/storage/songxuemeng/lizekun/ExIBR/valid_ijk_811_noshuffle_10000.txt", "r"),
                         delimiter="\t", skiprows=0)
...
write_file = open('/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_valid8110_10000.pkl',
                  'wb')
# cPickle.dump将train数据放入pkl文件，其中train又包含三个矩阵，第一个为top的feature,第二个为和top相搭配的bottom feature，
# 第三个为随意搭配的bottom feature.
...
print   ('now():' + str(datetime.now()))
ijk_list = numpy.loadtxt(open("/storage/songxuemeng/lizekun/ExIBR/test_ijk_811_noshuffle_10000.txt", "r"),
                         delimiter="\t", skiprows=0)
...
write_file = open('/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_test8110_10000.pkl',
                  'wb')
# cPickle.dump将train数据放入pkl文件，其中train又包含三个矩阵，第一个为top的feature,第二个为和top相搭配的bottom feature，
# 第三个为随意搭配的bottom feature.
