...
class dA_joint(object):
    ...
    def get_cost_updates(self, corruption_level, learning_rate):
        ...
        #     d_x1_x2 = T.nnet.hard_sigmoid (T.dot(y1, y2t))
        #     d_x1_x3 = T.nnet.hard_sigmoid (T.dot(y1, y3t))
        #     d_x1_x2 = 1 / (1 + T.exp(T.dot(y1, y2t)))
        #     d_x1_x3 = 1 / (1 + T.exp(T.dot(y1, y3t)))
...
#        L_mod = T.mean(T.nnet.hard_sigmoid(T.dot(y1t_v, y1_c))) + T.mean(T.nnet.hard_sigmoid(T.dot(y2t_v, y2_c))) + T.mean(T.nnet.hard_sigmoid(T.dot(y3t_v, y3_c)))
...
def test_dA(learning_rate=0.1, batch_size=128, epoch_time=30, max_patience=3):

    fb = open('best_vt10_100_my_SIGMOID.txt', 'a+')
    fi = open('info_vt10_100_my_SIGMOID.txt', 'a+')


    print 'loading data'

    print 'loading train VISUAL data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_train_811_norm.pkl", "rb") as f:
        train_set =  numpy.asarray(cPickle.load(f),dtype='float32')

    print 'loading train TEXT data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_train8110.pkl", "rb") as f:
        train_txt_set =  numpy.asarray(cPickle.load(f),dtype='float32')


    print 'loading valid VISUAL data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_valid_811_norm.pkl", "rb") as f:
        valid_set =  numpy.asarray(cPickle.load(f),dtype='float32')


    print 'loading valid Text data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_valid8110.pkl", "rb") as f:
        valid_txt_set = numpy.asarray(cPickle.load(f),dtype='float32')

    print 'loading test visual  data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_test_811_norm.pkl", "rb") as f:
        test_set = numpy.asarray(cPickle.load(f),dtype='float32')

    print 'loading test text data'
    print('now():' + str(datetime.now()))
    with open("/storage/songxuemeng/lizekun/ExIBR/AUC_new_dataset_unified_text_test8110.pkl", "rb") as f:
    ...
    for _lamda in range(-2, -1, 1):
        for _mu in range(-2, -1, 1):
            for _beta in range( -3, -2, 1):
                for n_hidden in [1024, 512, 256]:
               # for n_hidden in [2,4,8,16,64,128,256,512]:
                    index = T.lscalar()
                    xi_v = T.matrix('xi_v')
                    xj_v = T.matrix('xj_v')
                    xk_v = T.matrix('xk_v')
                    ...
                    last_train_model = theano.function(
                        [],
                        outputs=da.get_cost_updates(corruption_level=0.3,learning_rate=learning_rate)[0],
                        givens={
                            xi_v: train_set_xi_v,
                            xj_v: train_set_xj_v,
                            xk_v: train_set_xk_v,
                    ...
                    def valid_model(size, k):
                        ...
                        vi_list = vi_v.tolist()
                        vj_list = numpy.transpose(vj_v).tolist()
                        vk_list = numpy.transpose(vk_v).tolist()
                        top = []
                        performance = 0.0
                        for i in range(size):
                        ...
                        for i in range(top.__len__()):
                            ...
                            vij = vij_v[i][positive[0]]
                            cij = vij_c[i][positive[0]]
                            ij = vij + cij
                            compare.append(ij)
                            k_list = random.sample(range(100), 3*k)
                            for j in k_list:
                    ...
                    def test_model(size, k):
                        ...
                        ti_list = ti_v.tolist()
                        tj_list = numpy.transpose(tj_v).tolist()
                        tk_list = numpy.transpose(tk_v).tolist()
                        top = []
                        performance = 0.0
                        for i in range(size):
                        ...
                        for i in range(top.__len__()):
                            ...
                            for j in range(size):
                                if (ti_list[j] == t_v):
                                    positive.append(j)
                                    positive_v.append(tj_list[j])
                            vij = tij_v[i][positive[0]]
                            cij = tij_c[i][positive[0]]
                            ij = vij + cij
                            compare.append(ij)
                            k_list = random.sample(range(100), 3*k)
                            for j in k_list:
                    ...
                    '''
                        ...
                        cost_sqr,
                        cost_123)
                        fi.write('lamda:%.10f, mu:%.10f, beta:%.10f, theta:%.10f, cost is %.10f, L_rec_cost is %.10f, L_sup_cost is %.10f, L_mod_cost is %.10f, L_sqr is %.10f, L_123 is %.10f  \n' % (
                            lamda, mu, beta, theta,
                            cost,
                            rec_cost,
                            sup_cost,
                        ...
                        this_validation_ndcg_score = valid_model(valid_set_size, 10)
                        ...
                        fi.write('lamda %.10f, mu %.10f, beta %.10f, theta %.10f, validation score is %.10f\n' % (
                            iter[0], iter[1],iter[2],iter[3],
                            this_validation_ndcg_score
                        ))
                        fi.flush()
                        ...
                        '''
                            ...
                            print 'test start'
                            test_ndcg_score, tij_v, tij_c, tik_v, tik_c = test_model(test_set_size, 10)
                        ...
                        fb.write(
                            'iteration ended, best_validation_ndcg_score is %f, test_ndcg_score is %f, best valid iteration is lamda %f,mu %f, hidden %f, beta %f, theta %f, learning rate %f, batchsize %f  time %s\n' % (
                            best_validation_ndcg_score,
                            test_ndcg_score, best_valid_iter[0] , best_valid_iter[1] ,
                            best_hidden, best_beta, best_theta, learning_rate, batch_size,
                            str(datetime.now())))

                        fb.flush()

                    '''
                    numpy.savetxt('./score/vt10_100_my_train_score.csv', train_score, fmt="%f")
                    numpy.savetxt('./score/vt10_100_my_valid_score.csv', valid_score, fmt="%f")
                    numpy.savetxt('./score/vt10_100_my_test_score.csv', test_score, fmt="%f")
                    numpy.savetxt('./score/vt10_100_my_last_cost.csv', last_train_cost, fmt="%f")
                '''
    end_time = time.clock()
    ...
    fi.write('running time is %.10f' % (end_time - start_time))
    fi.flush()
